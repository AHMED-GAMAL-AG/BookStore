...
class BookSeeder extends Seeder
{
    ...
    {
        $book1 = Book::create([
            'category_id' => Category::where('name', 'ريادة أعمال')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
        ...
        $book1->authors()->attach(Author::where(['name' => 'محمد العربي'])->first()); // add author محمد العربي to the table

        $book2 = Book::create([
            'category_id' => Category::where('name', 'العمل الحر')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
            'title' => 'دليلك لاحتراف العمل الحر',
        ...
        $book2->authors()->attach(Author::where('name', 'هيثم طلعت')->first());

        $book3 = Book::create([
            'category_id' => Category::where('name', 'تسويق ومبيعات')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
        ...
        $book3->authors()->attach(Author::where('name', 'أياد قنديبي')->first());

        $book4 = Book::create([
            'category_id' => Category::where('name', 'العمل الحر')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
            'title' => 'دليلك المختصر لبدء العمل عبر الإنترنت',
            'description' => 'تحية إلى كل الأحرار … لا، هذه ليست دعوة ثورية ضد نظام الحكم للأسف كما قد يخطر ببالك، ولكنها دعوة إلى الثورة على نمط الحياة الممل الذي يحياه معظمنا.
        ...
        $book4->authors()->attach(Author::where('name', 'ياسر سلامة')->first());

        $book5 = Book::create([
            'category_id' => Category::where('name', 'تصميم')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
            'title' => 'الدليل المختصر لصفحات الهبوط',
            'description' => 'صفحات الهبوط أو Landing Pages أصبحت الآن من الثوابت في عالم التسويق الإلكتروني لكل من يرغب في بناء قائمة بريدية ثرية من العملاء المستهدفين، الذين يمثلون أعلى عائد على الاستثمار ROI (اختصارًا لـ Return On Investment) إذا تم استغلالها بالشكل الصحيح.
        ...
        $book5->authors()->attach(Author::where('name', 'هاشم سليم')->first());

        $book6 = Book::create([
            'category_id' => Category::where('name', 'برمجة')->first()->id,
            'publisher_id' => Publisher::where('name', 'أحمد جمال')->first()->id,
            'title' => 'دليلك المختصر للعمل كمطور ويب',
        ...
        $book6->authors()->attach(Author::where('name', 'فاطمة حيشية')->first());
    }
}
